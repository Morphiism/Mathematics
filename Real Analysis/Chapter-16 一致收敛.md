...
**定义 16.3.1** $L^{\infty}$ 度量，$L^{\infty}$ 范数
...
$B(X,Y)$ 上的 $L^{\infty}$ **范数**或**一致范数**定义为

$$
\lVert f \rVert _{\infty}=d_{\infty}(f,0)=\sup_{x \in X}d_{Y}(f(x),0)
$$

根据上面的分析，$(B(X,Y),d_{\infty})$ 就是一个度量空间（或赋范空间），并且函数序列 $(f^{(n)})$ 一致收敛于 $f$ 当且仅当 $(f^{(n)})$ 按度量 $d_{\infty}$ 收敛于 $f$. 这就解释了为什么 $X$ 中的序列极限与 $B(X,Y)$ 上的一致极限如此相似。
...
**命题 16.4.3** 几何级数（geometric series）
