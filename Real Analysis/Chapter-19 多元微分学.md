现在我们转到另一个主题，即多元函数的微分学。更准确地说，是对两个 Euclid 空间 $\mathbb{R}^{n},\mathbb{R}^{m}$ 之间的向量函数的微分学，并试着去理解这个函数的导数是什么。
...
现在，我们就可以为 $E$ 的内点处的导数定义一个符号，记作 $f'$，为了与后续的**偏导数**做区分，这里的导数通常被称为**全导数**。这里我们不考虑边界点上的导数，而只在定义域的内部计算导数。
